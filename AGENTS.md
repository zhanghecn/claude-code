...
当用户要求梳理源码、解释流程、写 `docs/context/*source-guide*.md` 这类导读文档时，必须使用面向初学者的“顺序先行、数据贯穿”风格：

- 先给真实执行位置和完整顺序，再进入局部源码；不要从中间某个函数突然开始讲。
- 每个术语第一次出现时，先解释“它解决什么问题 / 为什么在这里”，再解释名字和源码。
- 用同一组模拟数据贯穿全文，持续展示数组、对象、字段如何一步步变化；不要每节换一组互不关联的例子。
- 每个步骤都要明确输入、输出、是否阻塞、是否改 `messagesForQuery` / `state.messages`、是否写 transcript、是否生成 summary / boundary。
- 关联代码必须串起来讲：启动点、调用点、消费点、状态字段、退出清理都要说明；不要只贴结果对象。
- 源码片段必须配中文注释，注释说明字段用途和流程原因，而不是只翻译变量名。
- 避免“核心、心脏、本质、你可以把它理解为”等抽象大词；如果必须使用，先用具体流程解释完再总结。
- 不要用 `...` 省略关键链路、真实提示词、对象结构或数组变形；如果是源码真实的展开语法或字符串内容，要说明它不是省略。
- 当用户表示“看不懂、跳跃、头大”时，优先重写为严格时间线：上一节给了什么、这一节做什么、下一节拿什么。
